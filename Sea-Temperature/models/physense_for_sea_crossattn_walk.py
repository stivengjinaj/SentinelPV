...
class RealTimestepEmbedder(nn.Module):
    """
    Embeds scalar timesteps into vector representations.
    """
    def __init__(self, hidden_size, frequency_embedding_size=256):
    ...
    def forward(self, t):
        t_freq_1 = self.timestep_embedding(t[:,0], self.frequency_embedding_size)
        t_freq_2 = self.timestep_embedding(t[:,1], self.frequency_embedding_size)
        
        
        t_emb = self.mlp(torch.concat((t_freq_1, t_freq_2), dim=-1))
        return t_emb
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, s):
        ...
        if torch.isnan(q).any():
            print('find nan cross q:', q)
        
        if torch.isnan(k).any():
            print('find nan cross k:', k)
            
        if torch.isnan(v).any():
            print('find nan cross v:', v)
...
class Model(nn.Module):
    def __init__(self, patch_size=4, dim=374, depth=12, heads=8, mlp_dim=374, pool = 'cls', in_channels = 1, out_channels=1, dim_head = 32, dropout = 0., emb_dropout = 0.,args=None):
        super().__init__()
        image_height, image_width = (171, 360)
        ...
        self.time_encoder = RealTimestepEmbedder(dim // 4, frequency_embedding_size=dim // 4)
        ...
        self.mlp_head = FinalLayer(mlp_dim, patch_height=patch_height, patch_width=patch_width, out_channels=out_channels)

    def forward(self, x, t, time, row_indices, col_indices, sparse_sensor_field, sparse_pos):
        ...
        time = self.time_encoder(time)[:,None,:]
        time = time.repeat(1, n, 1)
        
        t = t + time
        
        
        sensor_feature = torch.concat([sparse_pos, sparse_sensor_field], dim=-1)
    ...
    def get_grid(self, shape, device):
        ...
        gridy = torch.tensor(np.linspace(0, 2, size_y), dtype=torch.float)
